...
.controller(
		...
		[
				...
				'InventoryReports',
				'Page',
				function($scope,InventoryReports,Page) {

					Page.setTitle('Inventory status report');
					...
					$scope.pagedItems = [];
					$scope.currentPage = 0;
					$scope.download_link = Page.rootURL()+'/inventory/adjustment/report2?';
					$scope.showPager = true;

					InventoryReports.search($scope.filter, function(
							enrollments) {
						console.log("Loaded all enrollments "
								+ enrollments.length);
						$scope.enrollments = enrollments;
					...
					$scope.executeSearch = function() {
						...
						if($scope.filter.type != ''){
							$scope.download_link = $scope.download_link + 'type='+ $scope.filter.type;
							console.log($scope.download_link);
						}
						if($scope.filter.customerSite != '' && $scope.filter.customerSite != undefined){
							$scope.download_link = $scope.download_link + '&customerSiteId='+ $scope.filter.customerSite;
							console.log($scope.download_link);
						}
						InventoryReports.search($scope.filter, function(
								enrollments) {
